defmodule TypeCheck.Macros do
  ...
  ** (TypeCheck.TypeError) At test/type_check/macros_test.exs:32:
  ...
  defp type_fun_definition(name_with_params, type, module_name, overrides, kind) do
    ...
    {helper_params, type} = strip_type_param_underscores(params, type)
    ...
    quote generated: true, location: :keep do
      @doc false
      # Dialyzer flags this type-value constructor as no_return when the
      # type mentions literals or remote types. c.f. #188 / #85
      @dialyzer {:nowarn_function, [{unquote(name), unquote(arity)}]}
      def unquote({name, call_meta(name_with_params), helper_params}) do
        unquote_splicing(params_check_code)
        # import TypeCheck.Builtin
        unquote(type_expansion_loop_prevention_code(name_with_params))
        mfa = {unquote(module_name), unquote(name), unquote(helper_params)}
  ...
  # Elixir >= 1.20 requires type parameters that occur only once in the
  # typespec to be underscored (e.g. `@type! t(_a) :: term()`).
  # The hidden helper function generated by `type_fun_definition/5` binds
  # and uses its parameters, which would trigger
  # "underscored variable is used after being set" warnings for such names.
  # The underscore is only meaningful to the typespec attribute itself,
  # so the helper uses the same parameter names without the leading underscore.
  defp strip_type_param_underscores(params, type) do
    names = MapSet.new(params, &type_param_name/1)

    {renames, helper_params} =
      Enum.reduce(params, {[], []}, fn param, {renames, helper_params} ->
        name = type_param_name(param)
        base = ununderscored_name(name)

        if base != nil and not MapSet.member?(names, base) do
          {[{name, base} | renames], [replace_type_param_name(param, base) | helper_params]}
        else
          {renames, [param | helper_params]}
        end
      end)

    helper_params = Enum.reverse(helper_params)
    {helper_params, rename_type_param_vars(type, Enum.reverse(renames))}
  end

  defp type_param_name({name, _meta, context}) when is_atom(name) and is_atom(context), do: name
  defp type_param_name(_), do: nil

  defp replace_type_param_name({name, meta, context}, replacement)
       when is_atom(name) and is_atom(context),
       do: {replacement, meta, context}

  defp replace_type_param_name(param, _), do: param

  # `_name` -> `name`; names that are underscores only (`_`, `__`) stay as-is
  defp ununderscored_name(name) do
    case Atom.to_string(name) do
      "_" <> rest ->
        if valid_variable_name?(rest), do: String.to_atom(rest), else: nil
  ...
  defp valid_variable_name?(<<first, rest::binary>>) when first in ?a..?z do
    rest
    |> String.to_charlist()
    |> Enum.all?(&(&1 in ?a..?z or &1 in ?A..?Z or &1 in ?0..?9 or &1 == ?_))
  end

  defp valid_variable_name?(_), do: false

  defp rename_type_param_vars(type, []), do: type

  defp rename_type_param_vars(type, renames) do
    renames = Map.new(renames)

    Macro.prewalk(type, fn
      {name, meta, context} when is_atom(context) and is_map_key(renames, name) ->
        {Map.fetch!(renames, name), meta, context}
